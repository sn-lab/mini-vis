...
cs.expname = 'spatial_tuning_test1';
cs.directory = 'C:\Users\Matthew\Documents\Schaffer-Nishimura Lab\Visual Stimulation\Data';
cs.trial_duration = 6;
cs.conditions = [2 5 10 20 40]; %barwidth
cs.conditions2 = [20 8 4 2 1]; %num gratings
cs.num_reps = 3;
...
for rep = 1:cs.num_reps
    for trial = 1:cs.num_trials
        ...
        cs.rep = rep;
        cs.trial = trial;
        cs.param.barwidth = cs.conditions(order(rep,trial));
        cs.param.numgratings = cs.conditions2(order(rep,trial));
        cs = c_com(cs, 'Send-Parameters'); %send grating parameters to controller (this function takes ~1 second)
