...
switch command
    ...
    case 'Disconnect'
        for i=1:length(vs)
            ...
            fclose(vs(i).controller); %close connection to arduinoVis
        end
    ...
    case 'Start-Pattern'
        for i=1:length(vs)
            fwrite(vs(i).controller,101,'uint8'); %command to send pattern parameters to arduinoVis
    ...
    case 'Backlight-On'
        for i=1:length(vs)
            fwrite(vs(i).controller,141,'uint8'); %command to turn backlight on
        end
        
    case 'Backlight-Off'
        for i=1:length(vs)
            fwrite(vs(i).controller,142,'uint8'); %command to turn backlight off
        end
        
    case 'Display-Number'
        for i=1:length(vs)
            fwrite(vs(i).controller,151,'uint8'); %command to show a number on the display
            fwrite(vs(i).controller,param,'uint8'); %in this case, param is the number to be displayed (0-255)
        end
